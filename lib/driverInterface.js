...
(function () {
    ...
    var driverInterface = function() {
        ...
        function initDriverHandle(driverHandleModulePath, driverHandleModuleName){
            if(loadedDrivers.indexOf(driverHandleModuleName) == -1){
                try {
                    console.log("Init driver handle " + driverHandleModuleName);
                    var driverHandle = require(path.join(driverHandleModulePath, driverHandleModuleName));
                    ...
                    loadedDrivers.push(driverHandleModuleName);
                    console.log("Successfully loaded module ["+driverHandleModuleName+"]");
                    return true;
                }
                catch(e) {
                    console.log('Error: cannot load driver handle ' + driverHandleModuleName);
                    console.log(e.message);
                }
            }
            else{
                console.log("Driver %s is already loaded.", driverHandleModuleName);
            }
            return false;
        }
        ...
        function loadDrivers() {
            ...
            for(var i in driverHandles) {
                if(driverHandles[i].indexOf("webinos-iot-driver-") == 0){
                    if(initDriverHandle(moduleLocation, driverHandles[i])){
                        driversLoaded = true;  //at least one driver is loaded
                    }
                }
            }
        }
